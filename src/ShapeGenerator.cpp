...
ShapeData ShapeGenerator::makePyramide()
{
    ShapeData ret;
...
ShapeData ShapeGenerator::makeTeapot(uint tesselation/*, const QMatrix4x4& lidTransform*/)
{
    ShapeData ret;
    QMatrix4x4 lidTransform = QMatrix4x4();
...
void ShapeGenerator::moveLid(int grid, float *v, const QMatrix4x4 &lidTransform) {
    ...
    bool reflectX, bool reflectY)
{
    ...
    buildPatch(patch, B, dB, v, n, tc, el,
        index, elIndex, tcIndex, grid, mat3, true);
...
void ShapeGenerator::buildPatch(QVector3D patch[][4],
    float *B, float *dB,
...
QVector3D ShapeGenerator::evaluate(int gridU, int gridV, float *B, QVector3D patch[][4])
{
...
QVector3D ShapeGenerator::evaluateNormal(int gridU, int gridV, float *B, float *dB, QVector3D patch[][4])
{
...
ShapeData ShapeGenerator::makeTorus(uint tesselation)
{
    ...
    for (uint round1 = 0; round1 < tesselation; round1++)
    {
        ...
        /*glm::rotate(QMatrix4x4(), round1 * sliceAngle, QVector3D(0.0f, 1.0f, 0.0f)) *
            glm::translate(QMatrix4x4(), QVector3D(torusRadius, 0.0f, 0.0f));*/
        QVector4D row0 = transform.row(0);
        QVector4D row1 = transform.row(1);
        QVector4D row2 = transform.row(2);

        /*float dataArray[9] = {row0[0],row0[1],row0[2],row1[0],row1[1],row1[2],row2[0],row2[1],row2[2]};
        float* data;
        data = dataArray;*/

        //QMatrix3x3 normalTransform = QMatrix3x3(data);
